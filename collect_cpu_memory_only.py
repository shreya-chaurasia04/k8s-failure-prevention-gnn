#!/usr/bin/env python3
"""
Collect ONLY CPU and Memory stress data.
Uses the updated 7-feature collector for new scenarios.
"""
...
def collect_batch(mode, label, num_batches=10, window_min=1):
    """Saves data with 7 features."""
    target_dir = FAILURE_DIR
    os.makedirs(target_dir, exist_ok=True)

    # 7-Feature Set with Node Exporter metrics
    queries = {
        ...
        "node_cpu": '100 - (avg by (node) (irate(node_cpu_seconds_total{mode="idle"}[1m])) * 100)',
...
def main():
    print("="*70)
    print("  COLLECTING CPU & MEMORY STRESS DATA ONLY")
    print("="*70 + "\n")
    ...
    print("\nNext steps:")
    print("   1. Train model: python3 structure/trainer/train.py")
